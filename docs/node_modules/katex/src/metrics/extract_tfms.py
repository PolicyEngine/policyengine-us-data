...
def find_font_path(font_name):
    try:
        ...
        raise RuntimeError(
            "Couldn't find kpsewhich program, make sure you"
            + " have TeX installed"
        )
    ...
    for family, chars in mapping.items():
        for char, char_data in chars.items():
            ...
            if family == "Script-Regular":
                tfm_char = font_name_to_tfm[font].get_char_metrics(
                    tex_char_num, fix_rsfs=True
                )
            else:
                tfm_char = font_name_to_tfm[font].get_char_metrics(
                    tex_char_num
            ...
            if (
                font_skewchar[font]
                and font_skewchar[font] in tfm_char.kern_table
            ):
                skewkern = round(tfm_char.kern_table[font_skewchar[font]], 5)
