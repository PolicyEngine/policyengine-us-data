...
scores = pd.read_csv(FOLDER / "scores.csv") if (FOLDER / "scores.csv").exists() else pd.DataFrame({
    "reform_id": [],
    "dataset": [],
    "year": [],
})
...
def get_budget(dataset: str, year: int, reform_id: int = None) -> float:
    ...
    dataset = {
        ds.name: ds
        for ds in [EnhancedCPS_2024, CPS_2024, PUF_2024]
    }[dataset]
    ...
    tax_revenues = sim.calculate("household_tax_before_refundable_credits", period=year).sum() - sim.calculate("household_refundable_tax_credits", period=year).sum()
...
def get_budgetary_impact(dataset: str, year: int, reform_id: int) -> float:
    ...
    if not scores[scores.reform_id == reform_id][scores.dataset == dataset][scores.year == year].empty:
        scores = scores.drop(scores[scores.reform_id == reform_id][scores.dataset == dataset][scores.year == year].index)
    scores = pd.concat([scores, pd.DataFrame({
        "reform_id": [reform_id],
        "dataset": [dataset],
        "year": [year],
        "budgetary_impact": [round((with_reform - baseline)/1e9, 1)]
    })])
    scores.to_csv(FOLDER / "scores.csv", index=False)

st.write("Use this page to compare the computed budgetary impacts of reforms by dataset.")

dataset = st.selectbox("Dataset", ["enhanced_cps_2024", "cps_2024", "puf_2024"])
...
scores_wide = scores[scores.reform_id == int(reform_id)].pivot(index="dataset", columns="year", values="budgetary_impact").fillna(0)
