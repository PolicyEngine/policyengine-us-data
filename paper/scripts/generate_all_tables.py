...
def generate_poverty_metrics():
    ...
    df = pd.DataFrame(data)
    return create_latex_table(
        df, caption="Poverty rates by dataset", label="tab:poverty_metrics"
    )
...
def generate_weight_stats():
    ...
    df = pd.DataFrame(data)
    return create_latex_table(
        df, caption="Weight distribution statistics", label="tab:weight_stats"
    )
...
def generate_target_examples():
    ...
    data = {
        "Source": ["IRS SOI", "Census", "CBO", "JCT", "Healthcare"],
        "Example Targets": [
