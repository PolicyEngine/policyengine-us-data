...
\subsection{Long-Run Projection and Trustees Assumptions}

For Social Security and other long-horizon applications, we extend the enhanced dataset beyond the near-term budget window by combining PolicyEngine parameter uprating with year-specific household reweighting. Each projection starts from the calibrated 2024 Enhanced CPS, applies the tax-benefit rules and economic indexing available for the target year, and then recalibrates household weights to the selected long-run target package. The long-run target package records its source and is written to each generated dataset's metadata, so downstream reform scores can be traced to the exact demographic, taxable payroll, Social Security benefit, and tax-on-benefits assumptions used in the baseline.

For analyses aligned to the 2025 Trustees reports, the default target source is \texttt{trustees\_2025\_current\_law}, drawing OASDI aggregates from the Social Security Trustees report and HI tax-on-benefits aggregates from the Medicare Trustees report \citep{ssaTrustees2025,cmsTrustees2025}. The publishable calibration profile targets single-year age counts, OASDI benefits, Social Security taxable payroll, and OASDI/HI taxation of benefits. We calculate taxable payroll as Social Security taxable wages plus taxable self-employment income, with a guardrail requiring the Social Security wage base to continue rising after 2035. This prevents a projection from silently using a parameter baseline whose taxable maximum has stopped indexing.

The federal income tax calculation used for long-run taxation-of-benefits targets is identified in metadata as \texttt{trustees-2025-core-thresholds-v1}. Under this assumption, the Social Security benefit-tax thresholds remain fixed in nominal dollars, while a core set of broader federal tax thresholds is wage-indexed after 2034. The core set includes ordinary income-tax brackets, the standard deduction and aged/blind addition, capital-gains thresholds, and AMT thresholds and exemptions. This is a modeling implementation of the Trustees-style long-run tax-side assumption; because the Trustees reports do not publish a line-by-line microsimulation parameter file for every federal tax threshold, we treat the assumption as an explicit scenario and retain a literal current-law indexing sensitivity for comparison.

Late-horizon projections can also face support limitations: combinations of age, Social Security benefits, taxable payroll, and tax-on-benefits liability that are rare in the 2024 CPS may become material after decades of demographic and wage growth. The projection pipeline therefore includes optional donor-backed support augmentation profiles. These profiles do not replace the base survey. Instead, they add tagged rows derived from real donor households or role-based donor composites, stamp the augmentation profile and build report into metadata, and audit realized age, benefit, payroll, and tax-on-benefits constraints after calibration. A long-run dataset is not treated as publishable solely because it is technically generated; it must also pass constraint-error, non-negative-weight, and weight-concentration checks.

For computationally expensive reform analysis, the same baseline construction can be paired with a sampled-year reporting workflow. In that workflow, the full microsimulation is run for the 10-year budget window and selected later years, and intervening annual reform effects are interpolated from those simulated years. Any such interpolation is disclosed with the reported results and kept separate from the baseline-generation metadata, which continues to identify the exact model version, target package, tax assumption, and support profile used for each simulated year.
...
\subsubsection{Long-Run Validation}

Long-run projection artifacts receive additional checks before they are used for policy scoring. For each projected year, we verify that the metadata identifies the base dataset, PolicyEngine model version, target source, calibration profile, tax assumption, and any support augmentation profile. We then compare the calibrated dataset to its target aggregates for age counts, OASDI benefits, taxable payroll, and taxation of benefits. We also inspect effective sample size and top-weight concentration to detect results that are technically feasible but driven by too few households. Finally, policy scores are compared against back-of-the-envelope estimates and external aggregates where available; large deviations trigger a diagnostic review before publication.
