...
\subsection{Long-Run Projection Outputs}

For long-run Social Security applications, validation focuses on the assumptions and aggregates needed for trust-fund analysis rather than only on the 2024 enhancement targets. The reported dashboard should identify the long-run target package, tax assumption, support augmentation profile, and interpolation approach used for each score. When the sampled-year workflow is used, tables should distinguish directly simulated years from interpolated years and should report 10-year and 75-year summaries separately. Numerical long-run results should be updated only after the corresponding baseline datasets, reform scores, and validation checks have been regenerated under the same assumption package.
