...
def partition_weighted_work_items(
    ...
    The current H5 pipeline represents work items as mappings with:
    - `type`
    - `id`
    - `weight`

    This helper stays compatible with that existing shape so the orchestration
    layer can adopt a tested partitioning seam before the request model is
    ...
    remaining = [
        item for item in work_items if work_item_key(item) not in completed
    ]
