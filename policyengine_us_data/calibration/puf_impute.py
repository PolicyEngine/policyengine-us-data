...
# Demographic predictors for QRF -- same as ExtendedCPS plus state_fips
...
# PUF variables to impute (from extended_cps.py)
IMPUTED_VARIABLES = [
    "employment_income",
...
# Variables where PUF values override CPS values in BOTH halves
...
def puf_clone_dataset(
    ...
    """
    # Determine record counts from data
    household_ids = data["household_id"][time_period]
    n_households = len(household_ids)

    # Find person-level variables by checking array lengths
    person_count = len(data["person_id"][time_period])
    persons_per_hh = person_count // n_households

    logger.info(
        "PUF clone: %d households, %d persons, " "%d persons/hh",
        n_households,
        person_count,
        persons_per_hh,
    )

    # Run QRF imputation if requested
    y_full = None
    y_override = None
    ...
    # Add state_fips to the dataset (doubled)
    new_data["state_fips"] = {
        time_period: np.concatenate([state_fips, state_fips]).astype(np.int32)
    }

    # Add any IMPUTED_VARIABLES that weren't in the original data
    if y_full:
...
def _run_qrf_imputation(
    ...
    Trains QRF on PUF data (with random state assignment) and
    predicts on CPS data using demographics + state_fips.
    ...
    # Build predictor names (without state for PUF calc, add later)
    demo_preds = [p for p in DEMOGRAPHIC_PREDICTORS if p != "state_fips"]

    # Build PUF training DataFrame
