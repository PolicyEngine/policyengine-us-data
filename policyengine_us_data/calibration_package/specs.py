...
class Stage2InputBundle:
    ...
    expected_source_dataset_size_bytes: int | None = None
    expected_target_database_sha256: str | None = None
    expected_target_database_size_bytes: int | None = None
    ...
    @property
    def expected_input_identities(self) -> dict[str, dict[str, str | int]]:
        """Return contract-declared Stage 2 input checksums and sizes."""

        identities: dict[str, dict[str, str | int]] = {}
        for label, expected_sha256, expected_size_bytes in (
            (
                "dataset",
                self.expected_source_dataset_sha256,
                self.expected_source_dataset_size_bytes,
            ),
            (
                "database",
                self.expected_target_database_sha256,
                self.expected_target_database_size_bytes,
            ),
        ):
            ...
            if expected_size_bytes is not None:
                identity["size_bytes"] = expected_size_bytes
            if identity:
    ...
    def validation_report(self) -> "ValidationReport":
        ...
        missing_labels = {label for label, _path in missing}
        for label, path, expected_sha256, expected_size_bytes in (
            (
                "dataset",
                self.source_dataset,
                self.expected_source_dataset_sha256,
            ...
            if expected_size_bytes is not None:
                ...
                if actual_size_bytes != expected_size_bytes:
                    findings.append(
                        ValidationFinding(
                            check_id=f"stage2_input_identity:{label}:size_bytes",
                            status="fail",
                            message=(
                                f"Stage 2 {label} artifact size mismatch: "
                                f"expected {expected_size_bytes}, "
                                f"found {actual_size_bytes} at {path}"
                            ),
                            metric="size_bytes",
                            value=actual_size_bytes,
                            threshold=expected_size_bytes,
                            metadata={
                                "artifact_label": label,
                                "path": str(path),
                                "source": self.source,
            ...
            if expected_sha256 is not None:
                actual_sha256 = f"sha256:{compute_file_checksum(path)}"
                if actual_sha256 != expected_sha256:
                    findings.append(
                        ValidationFinding(
                            check_id=f"stage2_input_identity:{label}:sha256",
                            status="fail",
                            message=(
                                f"Stage 2 {label} artifact checksum mismatch: "
                                f"expected {expected_sha256}, "
                                f"found {actual_sha256} at {path}"
                            ),
                            metric="sha256",
                            value=actual_sha256,
                            threshold=expected_sha256,
                            metadata={
                                "artifact_label": label,
                                "path": str(path),
                                "source": self.source,
    ...
    def _validation_metadata(self) -> dict[str, Any]:
        ...
        expected_identities = self.expected_input_identities
        if expected_identities:
            metadata["expected_identities"] = expected_identities
...
class Stage2InputBundleError(RuntimeError):
    ...
    def __init__(
        ...
        details = "; ".join(finding.message for finding in validation_report.findings)
        super().__init__(details or "Stage 2 input validation failed")
        self.bundle = bundle
...
def stage2_input_bundle_from_stage1_contract(
    ...
    *,
    ...
    source_artifact = _contract_artifact(
        contract,
        logical_names=_SOURCE_DATASET_LOGICAL_NAMES,
        label="source dataset",
    )
    target_artifact = _contract_artifact(
        contract,
        logical_names=_TARGET_DATABASE_LOGICAL_NAMES,
        label="target database",
    )
    source_dataset = _artifact_uri_to_path(source_artifact.uri)
    target_database = _artifact_uri_to_path(target_artifact.uri)
    ...
    return Stage2InputBundle(
        ...
        expected_source_dataset_sha256=getattr(source_artifact, "sha256", None),
        expected_source_dataset_size_bytes=getattr(
            source_artifact,
            "size_bytes",
            None,
        ),
        expected_target_database_sha256=getattr(target_artifact, "sha256", None),
        expected_target_database_size_bytes=getattr(
            target_artifact,
            "size_bytes",
            None,
        ),
...
def stage2_build_context_for_run(
    ...
    *,
    ...
    if stage1_contract_path is not None and not contract_path.exists():
        raise FileNotFoundError(f"Stage 1 contract not found: {contract_path}")
