...
def reweight(
    ...
    def loss(weights):
        ...
        mean_loss = rel_error_normalized.mean()
        l1_penalty = l1_lambda * torch.sum(weights)  # L1 on actual weights
        
        return mean_loss + l1_penalty
...
class L1_EnhancedCPS_2024(Dataset):
...
class ReweightedCPS_2024(Dataset):
    ...
    def generate(self):
        ...
        for year in [2024]:
            ...
            ## Unsure of this removal code, may have too high of time complexity:
            # ADD THIS: Remove households with very small weights
            threshold = 1e-6
            keep_mask = optimised_weights > threshold

            # Filter the dataset
            for key in data.keys():
                if isinstance(data[key], dict):
                    for year in data[key].keys():
                        if len(data[key][year]) == len(optimised_weights):
                            data[key][year] = data[key][year][keep_mask]
                elif len(data[key]) == len(optimised_weights):
                    data[key] = data[key][keep_mask]

            # Update the weights to only include non-zero ones
            data["household_weight"] = optimised_weights[keep_mask]

            # Optional: Rescale to maintain total weight
            scale_factor = original_weights.sum() / optimised_weights[keep_mask].sum()
            data["household_weight"] *= scale_factor
...
class EnhancedCPS_2024(EnhancedCPS):
    ...
    start_year = 2024
    end_year = 2024
    name = "L1_enhanced_cps_2024"
    label = "L1 Enhanced CPS 2024"
    file_path = STORAGE_FOLDER / "L1_enhanced_cps_2024.h5"
    url = "hf://policyengine/policyengine-us-data/L1_enhanced_cps_2024.h5"


if __name__ == "__main__":
    L1_EnhancedCPS_2024().generate()
