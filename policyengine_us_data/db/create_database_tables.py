...
class Stratum(SQLModel, table=True):
    ...
    stratum_group_id: Optional[int] = Field(
        default=None,
        description="Identifier for a group of related strata.",
    )
    notes: Optional[str] = Field(
        default=None,
        description="Descriptive notes about the stratum.",
    )
...
class StratumConstraint(SQLModel, table=True):
    ...
    notes: Optional[str] = Field(
        default=None,
        description="Optional notes about the constraint.",
    )
    ...
    def validate_operation(cls, v):
        ...
        if v not in allowed_ops:
            raise ValueError(
                ...
                f"Must be one of: {', '.join(allowed_ops)}"
            )
        return v
...
class Target(SQLModel, table=True):
    ...
    variable: USVariable = Field(
        description=(
            "A variable defined in policyengine-us " "(e.g., 'income_tax')."
        ),
    )
    period: int = Field(
    ...
    reform_id: int = Field(
        default=0,
        description=(
            "Identifier for a policy reform scenario " "(0 for baseline)."
        ),
    )
    ...
    tolerance: Optional[float] = Field(
        default=None,
        description=(
            "Allowed relative error as a percent " "(e.g., 25 for 25%)."
        ),
    )
    notes: Optional[str] = Field(
        default=None,
...
class Source(SQLModel, table=True):
    ...
    name: str = Field(
        description=(
            "Name of the data source " "(e.g., 'IRS SOI', 'Census ACS')."
        ),
        index=True,
    )
...
class VariableGroup(SQLModel, table=True):
    ...
    name: str = Field(
        description=(
            ...
            "(e.g., 'age_distribution', 'snap_recipients')."
        ),
        index=True,
        unique=True,
    )
    category: str = Field(
        description=(
            "High-level category "
            "(e.g., 'demographic', 'benefit', 'tax', 'income')."
        ),
        index=True,
    )
    is_histogram: bool = Field(
        default=False,
        description=(
            "Whether this group represents a " "histogram/distribution."
        ),
    )
    is_exclusive: bool = Field(
        default=False,
        description=(
            "Whether variables in this group are " "mutually exclusive."
        ),
    )
    aggregation_method: Optional[str] = Field(
        default=None,
        description=(
            "How to aggregate variables in this group "
            "(sum, weighted_avg, etc.)."
        ),
    )
    display_order: Optional[int] = Field(
        default=None,
        description=(
            "Order for displaying this group in " "matrices/reports."
        ),
    )
    description: Optional[str] = Field(
        default=None,
...
class VariableMetadata(SQLModel, table=True):
    """Maps PolicyEngine variables to their groups and provides
    metadata."""
    ...
    units: Optional[str] = Field(
        default=None,
        description=(
            "Units of measurement " "(dollars, count, percent, etc.)."
        ),
    )
    is_primary: bool = Field(
        default=True,
        description=(
            "Whether this is a primary variable vs " "derived/auxiliary."
        ),
    )
    notes: Optional[str] = Field(
        default=None,
...
def calculate_definition_hash(mapper, connection, target: Stratum):
    """Calculate and set the definition_hash before saving a
    Stratum instance."""
