...
from policyengine_us_data.db.create_database_tables import FieldValidValues
...
def populate_field_valid_values(session: Session) -> None:
    ...
    # Static values for type field (sources table)
    source_type_values = [
        ("type", "administrative", "Administrative data sources"),
        ("type", "survey", "Survey data sources"),
        ("type", "synthetic", "Synthetic/generated data"),
        ("type", "derived", "Derived from other sources"),
        ("type", "hardcoded", "Values hardcoded into the system"),
    ]

    # Add all static values
    static_count = 0
    for field_name, valid_value, description in (
        operation_values + active_values + period_values + source_type_values
    ):
        session.add(
            FieldValidValues(
