...
class Stratum(Base):
    ...
    stratum_id = Column(Integer, primary_key=True, comment="Unique identifier for the stratum.")
    stratum_group_id = Column(Integer, comment="Identifier for a group of related strata.")
    parent_stratum_id = Column(Integer, comment="Identifier for a parent stratum, creating a hierarchy.")
    ...
    constraints = relationship("StratumConstraint", back_populates="stratum", cascade="all, delete-orphan")
    targets = relationship("Target", back_populates="stratum", cascade="all, delete-orphan")
...
class StratumConstraint(Base):
    ...
    stratum_id = Column(Integer, ForeignKey('stratum.stratum_id'), primary_key=True)
    constraint_variable = Column(String, primary_key=True, comment="The variable the constraint applies to (e.g., 'age').")
    value = Column(String, primary_key=True, comment="The value for the constraint rule (e.g., '25' or '[NY,NJ]').")
    operation = Column(String, primary_key=True, comment="The comparison operator (e.g., 'greater_than_or_equal', 'in').")
    notes = Column(String, nullable=True, comment="Optional notes about the constraint.")
...
class Target(Base):
    ...
    variable = Column(String, nullable=False, comment="The name of the data variable (e.g., 'income_tax').")
    period = Column(Integer, nullable=False, comment="The time period for the data, typically a year.")
    ...
    stratum_id = Column(Integer, ForeignKey('stratum.stratum_id'), nullable=False, index=True)
    
    reform_id = Column(Integer, default=0, nullable=False, comment="Identifier for a policy reform scenario (0 for baseline).")
    value = Column(Float, comment="The numerical value of the target variable.")
    source_id = Column(Integer, comment="Identifier for the data source.")
    active = Column(Boolean, default=True, nullable=False, comment="Flag to indicate if the record is currently active.")
