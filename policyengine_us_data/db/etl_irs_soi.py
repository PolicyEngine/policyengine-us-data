...
FILER_DEMOGRAPHIC_UNDER65_TARGET_SPECS = (
    ...
    },
    {
        ...
        "active": False,
        "inactive_reason": (
            "current calibration H5s have zero support for single filers with "
            "dependents"
        ),
        "constraints": [
            ("tax_unit_is_filer", "==", "1"),
        ...
        rows.append(
            {
                ...
                "inactive_reason": spec.get("inactive_reason"),
            }
        )
    ...
    rows.append(
        {
            ...
            "active": False,
            "inactive_reason": (
                "current calibration H5s do not populate "
                "head_is_dependent_elsewhere, so this row would currently be "
                "impossible"
            ),
        }
    )
...
def load_national_filer_demographic_cell_targets(
    ...
    for spec in ALL_FILER_DEMOGRAPHIC_TARGET_SPECS:
        ...
        notes = base_notes
        if not row["active"]:
            notes += f"; stored inactive because {row['inactive_reason']}"
        _upsert_target(
            session,
