...
def extract_national_targets():
    ...
    dict
        ...
        - tax_filer_targets: Tax-related variables requiring filer
          constraint
        - conditional_count_targets: Enrollment counts requiring
          constraints
    ...
    direct_sum_targets = [
        {
            ...
            "notes": "Alimony received - survey reported, "
            "not tax-filer restricted",
            "year": HARDCODED_YEAR,
        },
        {
            ...
            "value": 13e9,
            "source": "Survey-reported (post-TCJA grandfathered)",
            "notes": "Alimony paid - survey reported, "
            "not tax-filer restricted",
            "year": HARDCODED_YEAR,
        },
        {
        ...
        },
        {
            ...
            "notes": "Social security tips uprated 40% to account "
            "for underreporting",
            "year": HARDCODED_YEAR,
        },
    ]
    ...
    ssn_none_targets_by_year = [
        ...
        },
        {
            ...
            "ACS-based residual estimate",
            "notes": "Undocumented population estimate "
            "(published May 2025)",
            "year": 2023,
        },
        {
            ...
            "notes": "Undocumented population central estimate "
            "(~13-14 million)",
            "year": 2024,
        },
        {
            ...
            "person_count": 13.0e6,
            "source": "Reuters synthesis of experts",
            "notes": "Same midpoint carried forward - "
            "CBP data show 95% drop in border apprehensions",
            "year": 2025,
        },
    ]
    ...
    for variable_name in cbo_vars:
        try:
            ...
            print(
                f"Warning: Could not extract CBO parameter "
    ...
    try:
        ...
        print(f"Warning: Could not extract Treasury EITC " f"parameter: {e}")
        treasury_targets = []
...
def transform_national_targets(raw_targets):
    """
    Transform extracted targets into standardized format.
...
def load_national_targets(
    ...
    with Session(engine) as session:
        calibration_source = get_or_create_source(
            ...
            description="National calibration targets from "
            "various authoritative sources",
            url=None,
            notes="Aggregated from CMS, IRS, CBO, Treasury, "
            "and other federal sources",
        )

        us_stratum = (
            session.query(Stratum)
            .filter(Stratum.parent_stratum_id == None)  # noqa: E711
            .first()
        )

        if not us_stratum:
            raise ValueError(
                "National stratum not found. "
                "Run create_initial_strata.py first."
            )
        ...
        if not tax_filer_df.empty:
            ...
            for _, target_data in tax_filer_df.iterrows():
                ...
                if pd.notna(target_data.get("notes")):
                    notes_parts.append(target_data["notes"])
                notes_parts.append(
                    f"Source: " f"{target_data.get('source', 'Unknown')}"
                )
                combined_notes = " | ".join(notes_parts)
                ...
                if existing_target:
                    ...
                    existing_target.notes = combined_notes
                    print(
                        f"Updated filer target: " f"{target_data['variable']}"
                    )
                else:
                    target = Target(
                    ...
                    session.add(target)
                    print(f"Added filer target: " f"{target_data['variable']}")
        ...
        for cond_target in conditional_targets:
            ...
            if existing_stratum:
                ...
                if existing_target:
                    ...
                    print(
                        f"Updated enrollment target " f"for {constraint_var}"
                    )
                else:
                    new_target = Target(
                        ...
                        notes=(
                            f"{cond_target['notes']} | "
                            f"Source: {cond_target['source']}"
                        ),
                    )
                    session.add(new_target)
                    print(f"Added enrollment target " f"for {constraint_var}")
            else:
                new_stratum = Stratum(
                    parent_stratum_id=us_stratum.stratum_id,
                ...
                print(
                    f"Created stratum and target "
                    f"for {constraint_var} enrollment"
                )
        ...
        print(f"\nSuccessfully loaded {total_targets} " f"national targets")
        print(f"  - {len(direct_targets_df)} direct sum targets")
        print(f"  - {len(tax_filer_df)} tax filer targets")
        print(
            f"  - {len(conditional_targets)} enrollment count "
            f"targets (as strata)"
        )
