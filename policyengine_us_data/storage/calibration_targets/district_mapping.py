...
def fetch_block_population(state) -> pd.DataFrame:
    ...
    for attempt in range(max_retries):
        try:
            print(
                f"  Attempting download (attempt {attempt + 1}/{max_retries})..."
            )
            ...
            print(
                f"  Timeout/connection error, retrying in {wait_time} seconds..."
            )
...
def build_crosswalk_cd116_to_cd119():
    ...
    mapping_path = Path(CALIBRATION_FOLDER / "district_mapping.csv")
