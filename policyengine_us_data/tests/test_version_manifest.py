...
# -- upload_files_to_gcs return value test --------------------------
...
class TestUploadFilesToGcsReturnsGenerations:
    ...
    def test_returns_generations(self, mock_storage, mock_auth, tmp_path):
        ...
        file_a = tmp_path / "file_a.h5"
        file_a.write_bytes(b"data_a")
        file_b = tmp_path / "file_b.h5"
        file_b.write_bytes(b"data_b")

        blob_a = MagicMock()
        blob_a.generation = 99999
        blob_b = MagicMock()
        blob_b.generation = 88888

        mock_bucket.blob.side_effect = [blob_a, blob_b]

        result = upload_files_to_gcs(
...
# -- End-to-end upload creates registry test ------------------------
...
class TestEndToEndUploadCreatesRegistry:
    ...
    def test_creates_registry(
        ...
        assert registry_data["current"] == "1.72.3"
        assert len(registry_data["versions"]) == 1
        assert registry_data["versions"][0]["hf"]["commit"] == "abc123"
        assert "test.h5" in registry_data["versions"][0]["gcs"]["generations"]
