...
def upload_versioned_files_to_gcs(
    files_with_paths: List[Tuple[Path, str]],
    version: str,
    gcs_bucket_name: str = "policyengine-us-data",
) -> int:
    """
    Upload files to versioned paths in GCS.
    ...
    for local_path, rel_path in files_with_paths:
        ...
        blob_name = f"v{version}/{rel_path}"
        blob = bucket.blob(blob_name)
...
def upload_versioned_files_to_hf(
    files_with_paths: List[Tuple[Path, str]],
    version: str,
    hf_repo_name: str = "policyengine/policyengine-us-data",
    hf_repo_type: str = "model",
    ...
    """
    Upload files to versioned paths in HuggingFace with retry logic.
    ...
    for local_path, rel_path in files_with_paths:
        ...
        operations.append(
            CommitOperationAdd(
                path_in_repo=f"v{version}/{rel_path}",
    ...
    if commit_message is None:
        commit_message = (
            f"Upload {len(operations)} files for version {version}"
        )
    ...
    logging.info(
        f"Uploaded {len(operations)} files to HuggingFace {hf_repo_name} "
        f"at v{version}/ with retry support."
    )
    return len(operations)


def upload_manifest_and_latest(
    manifest: Dict,
    version: str,
    ...
    """
    Upload manifest.json to versioned path, then update latest.json.

    This is the final step that makes a version "live". The latest.json
    pointer is only updated after the manifest is successfully uploaded.

    Args:
        manifest: Manifest dictionary (from manifest.py)
        version: Current version string
        previous_version: Version being replaced (for history)
        gcs_bucket_name: GCS bucket name
        hf_repo_name: HuggingFace repository
        hf_repo_type: Repository type
    """
    from policyengine_us_data.utils.manifest import create_latest_pointer
    ...
    manifest_blob_name = f"v{version}/manifest.json"

    gcs_manifest_blob = bucket.blob(manifest_blob_name)
    gcs_manifest_blob.upload_from_string(
        manifest_json, content_type="application/json"
    )
    logging.info(f"Uploaded {manifest_blob_name} to GCS.")

    hf_create_commit_with_retry(
        api=api,
        operations=[
            CommitOperationAdd(
                path_in_repo=manifest_blob_name,
        ...
        repo_type=hf_repo_type,
        token=token,
        commit_message=f"Upload manifest for version {version}",
    )
    logging.info(f"Uploaded {manifest_blob_name} to HuggingFace.")

    previous_versions = None
    try:
        existing_latest_blob = bucket.blob("latest.json")
        if existing_latest_blob.exists():
            existing_latest = json.loads(
                existing_latest_blob.download_as_string()
            )
            previous_versions = existing_latest.get("previous_versions", [])
            if previous_version is None:
                previous_version = existing_latest.get("current_version")
    except Exception as e:
        logging.warning(f"Could not read existing latest.json: {e}")

    latest = create_latest_pointer(
        version=version,
    ...
    gcs_latest_blob = bucket.blob("latest.json")
    gcs_latest_blob.upload_from_string(
        latest_json, content_type="application/json"
    )
    logging.info("Updated latest.json in GCS.")
    ...
    hf_create_commit_with_retry(
        ...
        operations=[
            CommitOperationAdd(
                ...
                path_or_fileobj=latest_json.encode("utf-8"),
            )
        ],
        repo_id=hf_repo_name,
        repo_type=hf_repo_type,
        token=token,
        commit_message=f"Update latest.json to version {version}",
    )
    logging.info("Updated latest.json in HuggingFace.")

    logging.info(f"Version {version} is now live!")
