...
def build_loss_matrix(dataset: type, time_period):
    ...
    baseline_is_child = sim.calculate("is_child").values
    baseline_ctc_individual_maximum = sim.calculate("ctc_individual_maximum").values
    baseline_ctc_value = sim.calculate("ctc_value").values
    baseline_child_ctc_recipients = (baseline_is_child & (baseline_ctc_individual_maximum > 0) & (baseline_ctc_value > 0)).astype(float)
    ...
    reform_ctc_individual_maximum = reform_sim.calculate("ctc_individual_maximum").values
    reform_ctc_value = reform_sim.calculate("ctc_value").values
    reform_child_ctc_recipients = (reform_is_child & (reform_ctc_individual_maximum > 0) & (reform_ctc_value > 0)).astype(float)
    ...
    loss_matrix[label] = sim.map_result(
        ctc_recipient_difference, "person", "household"
    )
