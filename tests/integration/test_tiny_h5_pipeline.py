...
def test_deployed_regional_coordinator_builds_default_catalog_from_seeded_artifacts():
    _require_modal_tokens()

    run_id = _run_id("h5-default-catalog")
    seed = _function(HARNESS_APP_NAME, "seed_h5_case")
    inspect = _function(HARNESS_APP_NAME, "inspect_h5_outputs")
    cleanup = _function(HARNESS_APP_NAME, "cleanup_h5_case")
    coordinate = _function(LOCAL_AREA_APP_NAME, "coordinate_publish")
    ...
    try:
        seeded = seed.remote(run_id, "default_catalog_success")
        ...
        assert result["message"].endswith("Upload skipped.")
        assert (
            result["reuse_measurement"]["expected_outputs"]
            == seeded["expected_output_count"]
        )
        assert result["reuse_measurement"]["invalid_outputs"] == 0

        expected_paths = tuple(seeded["expected_output_paths"])
        inspection = inspect.remote(run_id, list(expected_paths))
        _assert_output_contract(inspection, expected_paths)
    finally:
        cleanup.remote(run_id)
