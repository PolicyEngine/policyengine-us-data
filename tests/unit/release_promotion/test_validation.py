...
def _validator(
    ...
    return ReleaseCandidateValidator(dependencies=fake_deps.as_dependencies())
...
def test_release_candidate_validator_passes_complete_candidate() -> None:
    ...
    assert [finding.status for finding in report.findings] == ["pass"] * 5
...
def test_release_candidate_validator_reports_missing_required_families() -> None:
    ...
    report = _validator(fake_deps).validate(
        _bundle(paths=("national/US.h5", "states/AL.h5", "districts/NC-01.h5")),
        files_with_paths=_manifest_files(
            ("national/US.h5", "states/AL.h5", "districts/NC-01.h5")
        ),
    )
