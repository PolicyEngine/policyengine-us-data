...
def test_married_demographic_cell_constraint_matches_matrix_membership():
    values = np.array([b"JOINT", b"SEPARATE", b"SURVIVING_SPOUSE", b"SINGLE"])

    np.testing.assert_array_equal(
        apply_op(values, "in", MARRIED_FILING_STATUS_CONSTRAINT_VALUE),
...
def test_allocate_national_filer_demographic_cell_targets_marks_dependents_inactive():
    ...
    assert not targets.loc["single_under65_1dep", "active"]
    assert not targets.loc["single_under65_2plusdep", "active"]
    ...
    assert "zero support" in targets.loc["single_under65_1dep", "inactive_reason"]
...
def test_workbook_overlay_loads_education_tax_credit_target(monkeypatch, tmp_path):
    db_uri, engine = _create_test_engine(tmp_path)
    ...
    def fake_get_tracked_soi_row(variable, requested_year, **kwargs):
        ...
        rows = {
            ...
            ("education_tax_credits", True): {
                "Year": 2023,
                "Value": 7_211_349.0,
                "SOI table": "Table 3.3",
            },
            ("education_tax_credits", False): {
                "Year": 2023,
                "Value": 7_554_668_000.0,
                "SOI table": "Table 3.3",
            },
        }
        return pd.Series(rows[(variable, count)])
    ...
    rows = builder._query_targets(
        {
            ...
            "domain_variables": ["education_tax_credits"],
        }
    )
    ...
    assert set(rows["period"].astype(int)) == {2023}
    assert set(rows["value"].astype(float)) == {7_211_349.0, 7_554_668_000.0}
...
def test_geography_file_aca_ptc_uses_total_ptc_code():
    assert {
        "code": "85770",
        "name": "aca_ptc",
        "breakdown": None,
    } in GEOGRAPHY_FILE_TARGET_SPECS
