...
print(f"- All overlap coefficients > 0.85: {(support_df['overlap_coefficient'] > 0.85).all()}")
...
print("-"*40)
print(accuracy_df[['qrf_mae', 'qrf_improvement_vs_hotdeck', 'qrf_improvement_vs_linear', 'coverage_90pct']].round(1).to_string())
...
print("\nSummary:")
print(f"- All correlation differences < 0.05: {(joint_df['correlation_diff'] < 0.05).all()}")
...
with open('validation/outputs/qrf_diagnostics_summary.txt', 'w') as f:
    ...
    f.write("-"*40 + "\n")
    f.write(f"Average overlap coefficient: {support_df['overlap_coefficient'].mean():.3f}\n")
    f.write(f"All overlap coefficients > 0.85: {(support_df['overlap_coefficient'] > 0.85).all()}\n")
    ...
    f.write("\n3. OUT-OF-SAMPLE ACCURACY\n")
    f.write("-"*40 + "\n")
    f.write(f"Average improvement vs hot-deck: {accuracy_df['qrf_improvement_vs_hotdeck'].mean():.1f}%\n")
    f.write(f"Average improvement vs linear regression: {accuracy_df['qrf_improvement_vs_linear'].mean():.1f}%\n")
    f.write(f"90% prediction interval coverage: {accuracy_df['coverage_90pct'].mean():.1%}\n")
    
    f.write("\n4. JOINT DISTRIBUTIONS\n")
    f.write("-"*40 + "\n")
    f.write(f"All correlation differences < 0.05: {(joint_df['correlation_diff'] < 0.05).all()}\n")
    f.write(f"Average correlation difference: {joint_df['correlation_diff'].mean():.3f}\n")
