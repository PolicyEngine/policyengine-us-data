...
def load_dataset_as_dataframe(dataset_class):
    ...
    for var in demographic_vars:
        try:
            if var == 'age':
                ...
                data['num_dependents'] = sim.calculate(var, 2024, map_to="tax_unit").values
            else:
                data[var] = sim.calculate(var, 2024, map_to="person").values
        except:
    ...
    for var in income_vars:
        try:
            ...
            data[var.replace('_income', '')] = sim.calculate(actual_var, 2024, map_to="person").values
        except:
            print(f"Could not extract {var}")
            data[var.replace('_income', '')] = np.zeros(len(sim.calculate("person_id", 2024).values))
...
def main():
    ...
    print("\nSummary:")
    print(f"- Average overlap coefficient: {support_results['overlap_coefficient'].mean():.3f}")
    print(f"- All overlap coefficients > 0.85: {(support_results['overlap_coefficient'] > 0.85).all()}")
    print(f"- Variables with SMD > 0.25: {(support_results['standardized_mean_diff'] > 0.25).sum()}")
    print(f"- All SMDs < 0.25: {(support_results['standardized_mean_diff'] < 0.25).all()}")
    print(f"- Variables with significant KS test (p<0.05): {(support_results['ks_pvalue'] < 0.05).sum()}")
    print(f"- All KS tests non-significant (p>0.05): {(support_results['ks_pvalue'] > 0.05).all()}")
    ...
    variance_results = calculate_variance_explained(puf_data, predictors, list(target_map.values()))
    
    print("R-squared values:")
    for display_name, actual_name in target_map.items():
        if actual_name in variance_results:
            print(f"- {display_name.capitalize()}: {variance_results[actual_name]*100:.0f}%")
    ...
    if valid_pairs:
        ...
        print("\nSummary:")
        print(f"- All correlation differences < 0.05: {(joint_results['correlation_diff'] < 0.05).all()}")
        print(f"- Average correlation difference: {joint_results['correlation_diff'].mean():.3f}")
    ...
    print("✓ Saved common support analysis to validation/outputs/common_support_analysis.csv")
    ...
    with open('validation/outputs/variance_explained.txt', 'w') as f:
        ...
        for display_name, actual_name in target_map.items():
            if actual_name in variance_results:
                f.write(f"{display_name.capitalize()}: {variance_results[actual_name]*100:.0f}%\n")
    print("✓ Saved variance explained results to validation/outputs/variance_explained.txt")
    
    if valid_pairs:
        joint_results.to_csv('validation/outputs/joint_distribution_tests.csv', index=False)
        print("✓ Saved joint distribution tests to validation/outputs/joint_distribution_tests.csv")
    ...
    with open('validation/outputs/qrf_diagnostics_summary.txt', 'w') as f:
        ...
        f.write("-"*40 + "\n")
        f.write(f"Average overlap coefficient: {support_results['overlap_coefficient'].mean():.3f}\n")
        f.write(f"All overlap coefficients > 0.85: {(support_results['overlap_coefficient'] > 0.85).all()}\n")
        f.write(f"All SMDs < 0.25: {(support_results['standardized_mean_diff'] < 0.25).all()}\n")
        f.write(f"All KS tests p > 0.05: {(support_results['ks_pvalue'] > 0.05).all()}\n\n")
        ...
        if valid_pairs:
            ...
            f.write("-"*40 + "\n")
            f.write(f"All correlation differences < 0.05: {(joint_results['correlation_diff'] < 0.05).all()}\n")
            f.write(f"Average correlation difference: {joint_results['correlation_diff'].mean():.3f}\n")
